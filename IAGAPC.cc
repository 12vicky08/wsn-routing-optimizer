...
struct Chromosome {
  ...
  Chromosome()
      : fitness(0.0), diversityScore(0.0), energyMetric(0.0), smoothMetric(0.0),
        coverageMetric(0.0), delayMetric(0.0) {}
};
...
private:
  ...
  void Crossover(const Chromosome &p1, const Chromosome &p2, Chromosome &c1,
                 Chromosome &c2, double currentAvgFitness,
...
IAGAPCEnhanced::IAGAPCEnhanced(NodeContainer nodes,
                               Ptr<EnergySourceContainer> energySources,
                               double w, double h)
    : m_nodes(nodes), m_energySources(energySources), m_areaWidth(w),
      m_areaHeight(h) {
...
std::vector<Point>
IAGAPCEnhanced::GenerateSplinePath(const std::vector<Point> &rps) const {
  ...
  for (size_t i = 0; i < rps.size() - 3; i++) {
    ...
    for (double t = 0; t <= 1; t += 0.05) {
      ...
      double x =
          0.5 *
          ((2 * rps[i + 1].x) + (-rps[i].x + rps[i + 2].x) * t +
           (2 * rps[i].x - 5 * rps[i + 1].x + 4 * rps[i + 2].x - rps[i + 3].x) *
               t2 +
           (-rps[i].x + 3 * rps[i + 1].x - 3 * rps[i + 2].x + rps[i + 3].x) *
               t3);

      double y =
          0.5 *
          ((2 * rps[i + 1].y) + (-rps[i].y + rps[i + 2].y) * t +
           (2 * rps[i].y - 5 * rps[i + 1].y + 4 * rps[i + 2].y - rps[i + 3].y) *
               t2 +
           (-rps[i].y + 3 * rps[i + 1].y - 3 * rps[i + 2].y + rps[i + 3].y) *
               t3);
...
double IAGAPCEnhanced::CalculateFitness(Chromosome &ind) {
  ...
  ind.energyMetric = minEnergy / (sqrt(energyVar) +
                                  EPSILON); // Maximize MinEnergy, Minimize Var
  ...
  // 4. Coverage Factor
  // Check how many nodes are within COMM_RANGE of at least one point on the
  // path
  int coveredNodes = 0;
  for (uint32_t i = 0; i < m_nodes.GetN(); ++i) {
  ...
  // Weighted Sum
  ind.fitness = (W_ENERGY * ind.energyMetric) + (W_SMOOTH * ind.smoothMetric) +
                (W_COVERAGE * ind.coverageMetric) + (W_DELAY * ind.delayMetric);
...
// Adaptive Crossover
void IAGAPCEnhanced::Crossover(const Chromosome &p1, const Chromosome &p2,
                               Chromosome &c1, Chromosome &c2, double f_avg,
                               double f_max) {
  ...
  for (int gen = 0; gen < MAX_GENERATIONS; ++gen) {
    ...
    for (const auto &ind : m_population) {
      if (ind.fitness > maxFit)
        maxFit = ind.fitness;
      if (ind.fitness > m_globalBest.fitness)
        m_globalBest = ind;
      sumFit += ind.fitness;
    }
